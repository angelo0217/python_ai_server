    ...
    for msg in task_result.messages:
        conversation.append({
            ...
            "tokens": {
                ...
                "total": getattr(msg.models_usage, "prompt_tokens", 0) + getattr(msg.models_usage, "completion_tokens", 0)
            } if msg.models_usage else {}
        })
    ...
    with open(output_dir / "conversation.txt", "w", encoding="utf-8") as f:
        ...
        for msg in task_result.messages:
            ...
            if msg.models_usage:
                ...
                f.write(f"· 總計tokens: {msg.models_usage.prompt_tokens + msg.models_usage.completion_tokens}\n\n")
        ...
        if msg.source == "programmer":
            ...
            while i < len(lines):
                ...
                if (line.startswith("#") and ("/" in line or "." in line)) or \
                   (("文件" in line or "檔案" in line or "file" in line.lower()) and (":" in line or "：" in line)):
                    if line.startswith("#"):
                        path_candidate = line.strip("#").strip()
                    else:
                        parts = line.replace("：", ":").split(":", 1)
                        if len(parts) > 1:
                            path_candidate = parts[1].strip().strip('"').strip("'").strip("`").strip()
                ...
                if line.startswith('```'):
                    ...
                    if i < len(lines) and code_block:
                        ...
                        # 判斷文件類型和路徑
                        if current_file_path and ("/" in current_file_path or "\\" in current_file_path or "." in current_file_path):
                            ...
                            if '/' not in file_path and '\\' not in file_path:
                                if file_path.endswith(('.py', '.txt')):
                                    ...
                                    if file_path.endswith('.css'):
                                        file_path = f"frontend/static/css/{file_path.split('/')[-1]}"
                            ...
                            # 根據內容和語言識別文件類型
                            is_backend = (
                                (language and ("python" in language.lower() or "py" in language.lower())) or
                                "fastapi" in code_content.lower() or 
                                "flask" in code_content.lower() or
                                "django" in code_content.lower() or
                                "import " in code_content.lower() or
                                "def " in code_content.lower() or
                                "@app" in code_content or
                                ("class " in code_content and ":" in code_content)
                            )
                            
                            is_frontend = (
                                (language and ("html" in language.lower() or "css" in language.lower() or "js" in language.lower() or "javascript" in language.lower())) or
                                "<html" in code_content.lower() or
                                "<body" in code_content.lower() or
                                "document." in code_content.lower() or
                                "fetch(" in code_content.lower() or
                                "function " in code_content.lower() or
                                "const " in code_content.lower() or
                                "let " in code_content.lower() or
                                "addEventListener" in code_content.lower()
                            )
                            
                            is_requirements = (
                                "requirements.txt" in (current_file_path or "") or
                                (code_content.count("\n") < 10 and "==" in code_content) or
                                (code_content.count("\n") < 10 and ">=" in code_content) or
                                (language and "requirements" in language.lower())
                            )
                            ...
                            if is_requirements:
                                ...
                                if extension == ".py":
                                    ...
                                    elif "config" in code_content.lower() or "settings" in code_content.lower():
                                ...
                                if "<html" in code_content.lower():
                                    file_path = "frontend/index.html"
                                elif "{" in code_content and ":" in code_content and ";" in code_content and not "function" in code_content.lower():
                                    file_path = "frontend/static/css/style.css"
                                elif "function" in code_content.lower() or "const " in code_content or "fetch(" in code_content:
                                    file_path = "frontend/static/js/app.js"
                                elif extension == ".html":
                                    file_path = "frontend/index.html"
                                elif extension == ".css":
                        ...
                        # 3. 處理數字編號開頭（如 "1. 配置文件"）
                        if any(file_path.startswith(f"{i}. ") or file_path.startswith(f"{i}.") for i in range(1, 10)):
...
async def main():
    ...
    parser.add_argument("--output", type=str, default="code_review_results", help="輸出結果的文件夾名稱")
