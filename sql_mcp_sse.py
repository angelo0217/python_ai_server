...
async def execute_query(query: str, params: Optional[Union[List, Dict]] = None) -> List[Dict[str, Any]]:
    """執行 SQL 查詢並返回結果。"""
    try:
...
def init_database() -> None:
    ...
    if not os.path.exists(DB_PATH):
        ...
        cursor.executemany('INSERT OR IGNORE INTO users (name, email, age) VALUES (?, ?, ?)', test_data)
...
def format_result(result: List[Dict[str, Any]]) -> str:
    ...
    # 計算每列的最大寬度
    widths = [max(len(header), max([len(row[i]) for row in rows])) for i, header in enumerate(headers)]
    ...
    data_rows = []
    for row in rows:
        data_rows.append('|' + '|'.join(f' {cell:<{widths[i]}} ' for i, cell in enumerate(row)) + '|')
